 ...
 * 2. Privileged-RPC block (this gateway). Credentials, settings mutations,
 *    preset management, desktop directory pickers/openers, and the dynamic
 *    plugin runner's execution surface never pass through the gateway, token
 *    or not; the read-only settings/describe is allowed because the web
 *    client's boot requires it. Since deepseek-harness 0.1.5 there is no
 *    method-specific loopback tier upstream ("every Host RPC method requires
 *    one browser session"), so this list is the PRIMARY boundary keeping the
 *    configuration plane on the computer, not merely defense in depth. The
 *    names use the 0.1.5 slash wire format (`/api/settings/update`); the
 *    pre-0.1.3 dotted format (`/api/settings.update`) must never return.
 ...
 *    machine's IPv4 literals via --trusted-host so ordinary RPC passes, and
 *    every request still needs the upstream browser-auth session (layer 4).
 * 4. Upstream browser-auth bootstrap. The upstream web server gates the index
 *    and all /api calls behind a launch-token/cookie session whose cookie is
 *    authority-bound. When the host half supplies this process's launch
 *    token, the gateway replays a 401'd GET index once with ?token= so
 *    upstream mints the cookie straight onto the paired device; without a
 *    token the 401 passes through and the device needs the token URL printed
 *    by dsh web once.
 *
...
import { createDiagnostics, diagnosticError, diagnosticPath } from './diagnostics.js'
...
/**
 * Privileged RPC the gateway never forwards. Names use the deepseek-harness
 * 0.1.5 slash-separated endpoint wire format: POST /api/<namespace>/<method>
 * (the client composes `${channel}/${endpoint}`). Since 0.1.5 removed the
 * method-specific loopback tier upstream, this list is the only boundary that
 * keeps the configuration plane (settings, credentials, presets) and the
 * host-desktop surfaces (directory picker, path openers, dynamic plugin host
 * halves) on the computer; a paired phone can work sessions but cannot reach
 * them. Keep it in sync when upstream adds Remote methods.
 */
const GATEWAY_READ_EXCEPTIONS = new Set([
  ...
  // remote devices may VIEW configuration (secrets stay redacted upstream).
  // Everything that mutates settings, touches credentials, probes the network
  // (llm/discoverModels), or executes host-side plugin code stays blocked
  // below.
  'settings/describe',
])

const PRIVILEGED_METHODS = new Set([
  // Agent presets: read exposes preset documents, copy/delete mutate them.
  // list/select stay open: list is roster metadata and select is ordinary
  // per-session usage.
  'agentPresets/read',
  'agentPresets/copy',
  'agentPresets/deletePreset',
  // Desktop directory picker: pops modal dialogs on the computer and walks
  // its filesystem (pre-0.1.3 host.pickDirectory).
  'directoryPicker/pick',
  'directoryPicker/list',
  'directoryPicker/createDirectory',
  // Opens a path with the OS default application on the computer
  // (pre-0.1.3 host.openPath). The capability probe canOpenWorkspacePath is
  // a harmless boolean and stays open.
  'session/openWorkspacePath',
  // Settings plane: documents, directories, and every mutation.
  'settings/openSettingsDocument',
  'settings/openAgentPresetDirectory',
  'settings/update',
  'settings/replace',
  'settings/mutate',
  // Credentials: describe included — remote devices never read credential
  // metadata, not just the values.
  'credentials/describe',
  'credentials/set',
  'credentials/unset',
  // Network probe against arbitrary provider endpoints.
  'llm/discoverModels',
  // Dynamic plugin runner: executing host halves, invoking plugin methods,
  // and approving/stopping runs stay on the computer. Read/report/inspect
  // surface (inventory, getClientCode, report*, resolve*, sync*) stays open
  // so phone-side plugin panels still render.
...
(function(){
  ...
  ;(async function() {
    ...
    var lines = []
    try {
      var res = await fetch('/api/settings/describe', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ type: 'client-request', rpcId: 'diag-probe', method: 'settings/describe', payload: {} }),
      })
      lines.push('fetch settings/describe -> HTTP ' + res.status)
    } catch (e) {
      lines.push('fetch settings/describe -> ERR ' + String(e && e.message).slice(0, 80))
    }
    await new Promise(function(r) { setTimeout(r, 500) })
    lines.push(await new Promise(function(resolve) {
      var ws
      try { ws = new WebSocket(location.origin.replace('http', 'ws') + '/api/remote.mux') } catch (e) { resolve('ws ERR ' + String(e && e.message).slice(0, 60)); return }
      var done = false
 ...
 * @param {{ launchToken?: string }} [options] - this process's dsh web launch
 *   token; lets the gateway bootstrap paired devices into the upstream
 *   browser-auth session (see the index-401 replay below).
 * @returns {(() => Promise<void>) | undefined} disposer that closes the listener
 *   and flushes diagnostics, or undefined when
 *   the gateway is disabled and nothing was started.
 */
export function startGateway(env = process.env, options = {}) {
  const launchToken = typeof options?.launchToken === 'string' && options.launchToken !== ''
    ? options.launchToken
    : undefined
  ...
  const diagnostics = createDiagnostics(env.DSH_MOD_GATEWAY_LOG === '0' ? undefined
    : env.DSH_MOD_GATEWAY_LOG || join(dirname(statePath), 'logs', 'dsh-mod-gateway.jsonl'), {
    onError: code => console.error('[dsh-mod-gateway] diagnostic log unavailable:', code),
  })
  const configuredTimeout = Number(env.DSH_MOD_GATEWAY_WS_HANDSHAKE_TIMEOUT_MS)
  const handshakeTimeoutMs = Number.isInteger(configuredTimeout) && configuredTimeout > 0 && configuredTimeout <= 2_147_483_647
    ? configuredTimeout : 15_000
  let nextId = 0, activeHttp = 0, stopping = false, shutdown
  const tunnels = new Set()
  const elapsed = start => Math.round(performance.now() - start)
  let lastSample = performance.now()
  const heartbeat = setInterval(() => {
    const now = performance.now()
    diagnostics.record('gateway.heartbeat', {
      activeHttp, activeWs: tunnels.size, eventLoopLagMs: Math.max(0, Math.round(now - lastSample - 30_000)),
    })
    for (const tunnel of tunnels) tunnel.sample()
    lastSample = now
  }, 30_000)
  heartbeat.unref()
  ...
  // Compare codes after stripping separators, so the dashed form printed in
  // the console (XXXX-XXXX) and the operator's DSH_MOD_GATEWAY_CODE with a
  // space or dash both match the stored alphanumerics.
  const normalizeCode = value => value.toUpperCase().replace(/[^A-Z0-9]/g, '')
  const codeMatches = candidate => typeof candidate === 'string'
    && safeEqual(sha256(normalizeCode(candidate.trim())), sha256(normalizeCode(state.pairingCode)))
  ...
  const server = createServer((req, res) => {
    ...
    const context = { id: ++nextId, method: req.method, path: diagnosticPath(req.url), peer: req.socket.remoteAddress }
    let upstreamStatus, upstreamMs, completed = false
    const requests = new Set()
    activeHttp++
    diagnostics.record('http.start', context)
    const complete = aborted => {
      if (completed) return
      completed = true
      activeHttp--
      diagnostics.record(aborted ? 'http.aborted' : 'http.complete', {
        ...context, status: res.headersSent ? res.statusCode : undefined,
        upstreamStatus, upstreamMs, durationMs: elapsed(started), slow: elapsed(started) >= 2000,
      })
      if (aborted) for (const request of requests) request.destroy()
    }
    res.once('finish', () => complete(false))
    res.once('close', () => complete(!res.writableFinished))
    req.once('aborted', () => { complete(true); res.destroy() })
    req.on('error', error => {
      diagnostics.record('http.error', { ...context, side: 'client', code: diagnosticError(error) })
      res.destroy()
    })
    const trackRequest = request => {
      requests.add(request)
      request.once('close', () => requests.delete(request))
      if (completed) request.destroy()
      return request
    }
    const upstreamFailure = (error, side = 'upstream') => {
      diagnostics.record('http.error', { ...context, side, code: diagnosticError(error), durationMs: elapsed(started) })
      if (completed) return
      if (res.headersSent) res.destroy()
      else { res.writeHead(502, { 'content-type': 'text/plain; charset=utf-8' }); res.end('gateway: upstream unavailable (is the DSH web server up?)') }
    }
    void (async () => {
      ...
      // exceptions per GATEWAY_READ_EXCEPTIONS). The method is the FULL
      // endpoint after /api/ — the 0.1.5 wire format is namespace/method
      // (`/api/settings/update`), so taking only the first path segment
      // would degrade the match to the namespace and never fire.
      const method = pathname.startsWith('/api/') ? pathname.slice('/api/'.length) : undefined
      ...
      // web client's boot reads; the gateway has already authenticated the
      // caller, so for exactly these methods the request is presented to the
      // upstream as a loopback request (Host and Origin rewritten to the
      // target). deepseek-harness 0.1.5 serves settings/describe to any
      // trusted host, so this is hardening rather than a requirement — it
      // keeps the boot read working even when the launcher was started
      // without the start script's --trusted-host entries. Everything else
      // keeps the original Host, and every mutating/secret method remains
      // blocked here (since 0.1.5 this list is the boundary, not a mirror).
      ...
      /**
       * Upstream browser-auth bootstrap: the upstream web server gates the
       * index and every /api call behind its launch-token/cookie session, and
       * that cookie is bound to the caller's authority — a freshly paired
       * phone always starts without one, so `/` would 401 with "dsh web
       * authentication required". When the plugin supplied this process's
       * launch token, answer an index 401 by replaying the GET once with
       * ?token= appended: upstream mints the authority-bound cookie and the
       * relayed 303 + Set-Cookie land on the phone, after which index, /api
       * and the WebSocket streams all pass. The token never leaves this
       * process except over the loopback hop to upstream.
       *
       * Two cookies are minted, not one: GATEWAY_READ_EXCEPTIONS requests are
       * presented to upstream with the Host rewritten to the loopback target
       * (see below), so a paired device also needs a session cookie bound to
       * THAT authority. The loopback exchange runs alongside the caller's own
       * and its Set-Cookie is merged into the relayed 303 — minted together,
       * expiring together, refreshed together.
       */
      let mintLoopbackCookies = []
      const exchangeLoopbackSession = tokenPath => new Promise(resolve => {
        const headers = { ...req.headers, host: `${target.host}:${target.port}` }
        if (req.headers.origin !== undefined) headers.origin = `http://${target.host}:${target.port}`
        const exchange = trackRequest(httpRequest(
          { host: target.host, port: target.port, method: 'GET', path: tokenPath, headers, agent: upstreamAgent },
          exchangeRes => {
            exchangeRes.on('error', error => { upstreamFailure(error, 'session'); resolve([]) })
            exchangeRes.resume()
            const cookies = exchangeRes.headers['set-cookie']
            resolve(Array.isArray(cookies) ? cookies : cookies === undefined ? [] : [cookies])
          },
        ))
        exchange.on('error', error => {
          diagnostics.record('http.error', { ...context, side: 'session', code: diagnosticError(error) })
          resolve([])
        })
        exchange.end()
      })
      const sendUpstream = (requestPath, isRetry) => {
        if (completed) return
        const sentAt = performance.now()
        const upstream = trackRequest(httpRequest(
          { host: target.host, port: target.port, method: req.method, path: requestPath, headers: upstreamHeaders, agent: upstreamAgent },
          upRes => {
            upstreamStatus = upRes.statusCode
            upstreamMs = elapsed(sentAt)
            upRes.on('error', upstreamFailure)
            if (!isRetry && (upRes.statusCode ?? 0) === 401 && req.method === 'GET'
              && pathname === '/' && launchToken !== undefined && !requestPath.includes('token=')) {
              upRes.resume()
              const joiner = requestPath.includes('?') ? '&' : '?'
              const tokenPath = `${requestPath}${joiner}token=${encodeURIComponent(launchToken)}`
              log(`minting upstream browser session for ${req.socket.remoteAddress ?? '?'} (index 401)`)
              void exchangeLoopbackSession(tokenPath)
                .then(loopbackCookies => {
                  mintLoopbackCookies = loopbackCookies
                  sendUpstream(tokenPath, true)
                })
              return
            }
            ...
            if (isRetry && mintLoopbackCookies.length > 0) {
              const existing = closeHeaders['set-cookie']
              closeHeaders['set-cookie'] = [
        ...
        upstream.on('error', upstreamFailure)
        // The retry is only ever a body-less GET index replay; the client
        // request was already consumed by the first attempt, and a request
        // nobody ends never leaves the agent — end it explicitly.
        if (isRetry) upstream.end()
        else req.pipe(upstream)
      }
      sendUpstream(req.url, false)
    })().catch(error => {
      diagnostics.record('http.error', { ...context, side: 'gateway', code: diagnosticError(error) })
      if (completed) return
  ...
  server.on('upgrade', (req, socket, head) => {
    const started = performance.now()
    const context = { id: ++nextId, path: diagnosticPath(req.url), peer: socket.remoteAddress }
    let pathname
    try { pathname = new URL(req.url, 'http://gateway.invalid').pathname }
    catch { socket.destroy(); return }
    // Attach before authentication/connect so an early client reset is handled.
    socket.on('error', error => diagnostics.record('ws.error', { ...context, side: 'client', code: diagnosticError(error) }))
    if (pathname.startsWith(GATEWAY_ROOT) || pathname === '/mod-gateway') {
      diagnostics.record('ws.rejected', { ...context, status: 404 })
      socket.destroy()
      return
    }
    if (!isAuthed(state, req)) {
      diagnostics.record('ws.rejected', { ...context, status: 401 })
      socket.destroy()
      return
    }
    diagnostics.record('ws.start', context)
    let done = false, firstSide, status, clientBytes = head?.length ?? 0, upstreamBytes = 0
    let lastActivity = performance.now(), responseLine = Buffer.alloc(0)
    const sample = () => diagnostics.record('ws.sample', {
      ...context, upstreamStatus: status, durationMs: elapsed(started),
      clientBytes, upstreamBytes, idleMs: elapsed(lastActivity),
    })
    const tunnel = { sample, stop: () => finish('gateway', 'shutdown') }
    const finish = (side, reason) => {
      if (done) return
      done = true
      clearTimeout(deadline)
      tunnels.delete(tunnel)
      diagnostics.record('ws.closed', {
        ...context, side: firstSide ?? side,
        reason: status !== undefined && status !== 101 && reason !== 'shutdown' ? 'handshake_rejected' : reason,
        upstreamStatus: status,
        durationMs: elapsed(started), clientBytes, upstreamBytes, idleMs: elapsed(lastActivity),
      })
      // closeAllConnections does not include upgraded sockets. Always dispose
      // BOTH directions, including a reset while the loopback connect is pending.
      socket.destroy()
      upstream.destroy()
    }
    const deadline = setTimeout(() => finish('upstream', 'handshake_timeout'), handshakeTimeoutMs)
    deadline.unref()
    // The byte counter must not consume early client frames before pipe() is
    // installed. Keep them buffered until the loopback connection is ready.
    socket.pause()
    ...
    tunnels.add(tunnel)
    socket.on('data', chunk => { clientBytes += chunk.length; lastActivity = performance.now() })
    upstream.on('data', chunk => {
      upstreamBytes += chunk.length
      lastActivity = performance.now()
      if (status !== undefined) return
      // The status line may span TCP chunks. Never retain/log header values or
      // WebSocket payloads, and cap the parser even for a malformed upstream.
      responseLine = Buffer.concat([responseLine, chunk.subarray(0, Math.max(0, 128 - responseLine.length))])
      const end = responseLine.indexOf('\r\n')
      if (end < 0 && responseLine.length < 128) return
      const match = /^HTTP\/1\.[01] (\d{3})/.exec(responseLine.toString('latin1'))
      status = match ? Number(match[1]) : 0
      responseLine = Buffer.alloc(0)
      diagnostics.record('ws.handshake', { ...context, upstreamStatus: status, durationMs: elapsed(started) })
      // Preserve rejected HTTP handshakes on the wire too. A rejecting server
      // that never closes is still bounded by the handshake deadline.
      if (status === 0) finish('upstream', 'handshake_rejected')
    })
    // Keep the deadline until the COMPLETE handshake headers arrive, not just
    // the status line. This listener stores only the last three delimiter bytes.
    let headerTail = Buffer.alloc(0), headerBytes = 0
    const checkHeaders = chunk => {
      headerBytes += chunk.length
      const bytes = Buffer.concat([headerTail, chunk])
      if (bytes.includes('\r\n\r\n')) {
        if (status === 101) clearTimeout(deadline)
        upstream.off('data', checkHeaders)
      } else if (headerBytes > 32 * 1024) finish('upstream', 'handshake_too_large')
      else headerTail = bytes.subarray(-3)
    }
    upstream.on('data', checkHeaders)
    upstream.on('error', error => {
      diagnostics.record('ws.error', { ...context, side: 'upstream', code: diagnosticError(error) })
      finish('upstream', 'error')
    })
    socket.on('error', () => finish('client', 'error'))
    socket.once('end', () => { firstSide ??= 'client'; finish('client', 'end') })
    upstream.once('end', () => { firstSide ??= 'upstream' })
    socket.once('close', () => finish('client', 'closed'))
    upstream.once('close', () => finish('upstream', 'closed'))
  })

  server.on('error', error => {
    diagnostics.record('gateway.error', { code: diagnosticError(error) })
  ...
  const startup = loadState(statePath)
    .then(loaded => {
      state = loaded
    ...
    .then(() => {
      ...
      server.listen(port, bind, () => {
        ready = true
        diagnostics.record('gateway.started', { port, targetPort: target.port, diagEnabled, handshakeTimeoutMs })
        if (diagnostics.path) log(`  diagnostics: ${diagnostics.path} (JSONL, 3 files x 5 MiB)`)
        ...
        if (launchToken === undefined) {
          log('  note: no dsh web launch token supplied; each device must once open the token URL printed by dsh web')
        }
      })
    })
    .catch(error => {
      clearInterval(heartbeat)
      diagnostics.record('gateway.error', { code: diagnosticError(error) })
      logError(`cannot start: ${String(error)}`)
    })
  ...
  return () => {
    ...
    stopping = true
    clearInterval(heartbeat)
    diagnostics.record('gateway.stopping', { activeHttp, activeWs: tunnels.size })
    for (const tunnel of tunnels) tunnel.stop()
    if (ready) log('shutting down')
    const closed = new Promise(resolve => server.close(resolve))
    server.closeAllConnections?.()
    upstreamAgent.destroy()
    shutdown = Promise.all([closed, startup]).then(() => diagnostics.flush())
    return shutdown
  }
}
